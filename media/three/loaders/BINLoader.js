	...
	class BINLoader extends THREE.Loader {
		...
		parse( points ) {
            ...
            for (var i = 0; i < numPoints; i++)
            {
				...
				if (this.ptFeats.includes('intensity')){
					const intensity = points[i * numFeats + 3];
					colors.push( intensity, 0, 0 );
				}
				
				else if (this.ptFeats.includes('r g b')){
				...
				// highlight
				else if (highlight && points[(i + 1) * numFeats - 1] > 0.5){
					const x = points[i * numFeats];
					const y = points[i * numFeats + 1];
					const z = points[i * numFeats + 2];
